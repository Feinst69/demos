...
```{r}
df <- read_excel("C:/Users/tanjo/Desktop/Laplateforme/Kick Off 2 Election/data_abs.xlsx")
...
for (i in seq_along(variables)) {
  p <- ggplot(combined_df, aes_string(x = "reorder(Department, txabs)", y = variables[i], fill = "group")) +
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),  # Incliner les noms des départements
      panel.grid.major = element_blank(),  # Enlever la grille majeure
      panel.grid.minor = element_blank()   # Enlever la grille mineure
    ) +
    scale_fill_manual(values = c("Faible taux" = "blue", "Fort taux" = "red"))  # Définir des couleurs pour chaque groupe
...
final_df <- cbind(clr_data, other_vars)
...
```{r}
p1 <- scaled_data[, -12]
p2 <- data_abs[, ncol(df), drop = FALSE] 
df_regression <- cbind(p1, p2)
head(df_regression)
```
...
```{r}
library(glmnet)
x <- model.matrix(txabs ~ ., data = df_regression)
y <- df_regression$txabs
...
```
```{r}
x <- model.matrix(txabs ~ ., data = df_regression)  # Variables indépendantes
y <- df_regression$txabs  # Variable dépendante
lasso_model <- glmnet(x, y, alpha = 1)  # alpha = 1 pour Lasso

# Récupérer les coefficients avec validation croisée
lassoCV <- cv.glmnet(x, y, alpha = 1)  # Ajuster le modèle avec validation croisée
coef_lasso <- coef(lassoCV, s = "lambda.min")  # Coefficients au meilleur lambda

# Convertir les coefficients en data frame pour la visualisation
coef_df <- as.data.frame(as.matrix(coef_lasso))  # Convertir en data frame
coef_df$Variable <- rownames(coef_df)  # Ajouter les noms de variables

# Assurez-vous de renommer correctement la colonne des coefficients
colnames(coef_df)[1] <- "Coefficient"  # Renommer la colonne pour un accès plus facile

# Filtrer les coefficients non nuls
coef_df <- coef_df[coef_df$Coefficient != 0, ]  # Garder seulement les variables non nulles

# Vérifiez le contenu de coef_df pour comprendre la structure
print(coef_df)
...
ggplot(coef_df, aes(x = reorder(Variable, Coefficient), y = Coefficient)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Contribution des variables dans le modèle Lasso",
       x = "Variables",
       y = "Coefficient") +
  theme_minimal()
```
...
model_rf <- randomForest(txabs ~ ., data =df_regression, ntree = 500)
model_rf
```
```{r}
...
cat("Nombre de colonnes:", ncol(importance_df), "\n")

# Affichez les noms des colonnes
print(colnames(importance_df))

# Ajouter une colonne pour les noms de variables
importance_df$Variable <- rownames(importance_df)

# Renommer les colonnes pour faciliter la visualisation
# Vérifiez le nombre de colonnes et renommez correctement
if (ncol(importance_df) == 2) {
    colnames(importance_df) <- c("IncNodePurity", "IncMSE")  # Ajustez selon vos colonnes
} else if (ncol(importance_df) == 3) {
    colnames(importance_df) <- c("IncNodePurity", "IncMSE", "Variable")  # Utilisez les 3 si elles existent
}
...
```{r}
# Vérifier la structure de importance_df
str(importance_df)

# S'assurer que la colonne Variable existe et contient les bons noms
if(!"Variable" %in% colnames(importance_df)) {
  # Renommer la colonne si nécessaire
  importance_df$Variable <- rownames(importance_df)
}

# Visualiser l'importance des variables avec ggplot2
ggplot(importance_df, aes(x = reorder(Variable, IncNodePurity), y = IncNodePurity)) +  # Utilisez -IncNodePurity pour le tri décroissant
...
# Je veux faire une représentation en 3D
```{r}
library(plotly)
# Effectuer une ACP pour réduire à 3 dimensions
pca_result <- PCA(scaled_data, ncp = 3, graph = FALSE)

# Extraire les coordonnées des trois premières dimensions de l'ACP
df_pca <- as.data.frame(pca_result$ind$coord[, 1:3])
