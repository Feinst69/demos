...
```
```{r}
file_path <- "C:/Users/tanjo/Desktop/Laplateforme/Kick Off 2 Election/data_abs.xlsx"
...
Sur la base de ces contributions aux trois premières composantes, vous pouvez conserver les variables suivantes :

Taux de Pauvreté (TxPauv) : Contribution élevée à PC1.
Ouvrier et Artisant : Contribuent à PC1 et PC3.
Agriculteur (Agri) : Contribution significative à PC2.
Profession Indépendante (PI) et Employé : Contribuent fortement à PC2.
Salaire Moyen (Salairemoy) : Contribution élevée à PC3.
Cadres : Présente dans PC3.
### Visualisation en 3D
```{r}
...
# Définir une liste avec les combinaisons de dimensions
dim_combinations <- list(
  list(x = "PC1", y = "PC2", filename = "clusters_Dim1_Dim2_polygon.png"),
  list(x = "PC1", y = "PC3", filename = "clusters_Dim1_Dim3_polygon.png"),
  list(x = "PC2", y = "PC3", filename = "clusters_Dim2_Dim3_polygon.png")
)

# Boucle pour générer et sauvegarder les graphiques pour chaque combinaison de dimensions
for (dim in dim_combinations) {
  
  # Générer les polygones pour chaque cluster sur les dimensions en cours
  get_convex_hull <- function(data) {
    data[chull(data[[dim$x]], data[[dim$y]]), ]
  }
  
  polygons <- pca_data %>% group_by(cluster) %>% group_modify(~ get_convex_hull(.))
  
  # Créer le graphique avec les enveloppes convexes
  plot <- ggplot(pca_data, aes_string(x = dim$x, y = dim$y, color = "cluster")) +
    geom_point(size = 3, alpha = 0.8) +
    geom_polygon(data = polygons, aes_string(x = dim$x, y = dim$y, group = "cluster", fill = "cluster"), 
                 alpha = 0.2, color = "black", linetype = "dotted") +
    labs(title = paste("Clusters projetés sur les Dimensions", dim$x, "et", dim$y, "(avec polygones)"),
         x = paste("Dimension", dim$x), y = paste("Dimension", dim$y)) +
    theme_minimal() +
    theme(legend.position = "right")
  
  # Sauvegarder le graphique
  ggsave(filename = dim$filename, plot = plot, width = 10, height = 6)
  print(plot)
  
  # Afficher un message pour indiquer que l'image a été sauvegardée
#  message(paste("Graphique sauvegardé sous :", dim$filename))
}
```

### Variables dans les clusters
```{r}
# Vérifier les colonnes présentes et filtrer celles qui existent réellement dans le dataframe
variables <- c("Artisant", "Agri", "Ouvrier", "Employe", "PI", "Cadres", "NonDiplome", "txcho", "TxPauv", "Salairemoy", "HLM")
variables_existantes <- variables[variables %in% colnames(pca_data)]

# Calculer les moyennes et écarts-types par cluster pour chaque variable existante
summary_stats <- pca_data %>%
  group_by(cluster) %>%
  summarise(across(all_of(variables_existantes), list(mean = ~mean(.), sd = ~sd(.)), .names = "{col}_{fn}"))

# Afficher le tableau récapitulatif
print(summary_stats)

# Sauvegarder les statistiques descriptives dans un fichier CSV
#write.csv(summary_stats, "summary_stats_par_cluster.csv", row.names = FALSE)

# Message de confirmation
#message("Tableau des statistiques descriptives sauvegardé sous : summary_stats_par_cluster.csv")


```
```{r}
# Transformer les données en format long
means_long <- summary_clusters %>%
  pivot_longer(cols = -cluster,       # Toutes les colonnes sauf `cluster`
               ...
               values_to = "mean_value") # Nommer la colonne des valeurs correspondantes

# Voir les données transformées en format long pour vérifier la transformation
print(head(means_long))
```
